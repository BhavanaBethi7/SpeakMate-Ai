...
import { computeAdaptiveWeek, extractSkillSnapshot } from "@/lib/adaptive-progress";
...
export async function POST(request: Request) {
  ...
  if (durationSeconds < 15 || durationSeconds > 180) {
    return NextResponse.json({ error: "Recording should be between 15 seconds and 3 minutes." }, { status: 400 });
  }

  try {
    const { transcript, analysis, usedWhisper, usedGPT } = await processSpeechAssessment({
      topic, durationSeconds, clientTranscript, audio,
    });

    const progress = await ensureUserProgress(user.id);

    // FEATURE 4: Load last 3 assessments for adaptive week calculation
    ...
    const recentSnapshots = recentAssessments
      .map((a) => parseAssessmentScores(a.scores))
      .filter(Boolean)
      .map((a) => extractSkillSnapshot(a!));

    const allSnapshots = [extractSkillSnapshot(analysis), ...recentSnapshots];
    const adaptive = computeAdaptiveWeek(progress.currentWeek, allSnapshots);
    ...
    await prisma.userProgress.update({
      where: { userId: user.id },
      data: { currentWeek: adaptive.nextWeek, lastAssessmentAt: new Date() },
    });

    await recordPracticeActivity(user.id);

    return NextResponse.json({ id: assessment.id, analysis, adaptive, meta: { usedWhisper, usedGPT } });
  } catch (error) {
